...
const DIALOG_SOUND_PATH := "res://sounds/dialog.mp3"
...
var _dialog_sound_player: AudioStreamPlayer
...
func _setup_dialog_sound() -> void:
	_ensure_audio_bus("SFX")
	var stream := load(DIALOG_SOUND_PATH) as AudioStream
	if stream == null:
		return
	_dialog_sound_player = AudioStreamPlayer.new()
	_dialog_sound_player.stream = stream
	_dialog_sound_player.bus = "SFX"
	_dialog_sound_player.volume_db = -6.0
	add_child(_dialog_sound_player)


func _play_dialog_sound() -> void:
	if _dialog_sound_player == null:
		return
	_dialog_sound_player.play()
